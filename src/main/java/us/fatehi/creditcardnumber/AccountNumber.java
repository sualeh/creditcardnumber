...
public class AccountNumber extends BaseRawData implements RawData, Serializable {
  ...
  public AccountNumber(final String rawAccountNumber) {
    ...
    issuerIdentificationNumber =
        new DisposableStringData(rightPad(left(accountNumberString, 6), 6, "0"));
    lastFourDigits = new DisposableStringData(leftPad(right(accountNumberString, 4), 4, "0"));
  ...
  /**
   * Wipes the Issuer Identification Number from memory. Following recommendations from the <a href=
   * "http://docs.oracle.com/javase/6/docs/technotes/guides/security/crypto/CryptoSpec.html#PBEEx">Java
  ...
  /**
   * Wipes the last 4 digits of the primary account number from memory. Following recommendations
   * from the <a href=
   * "http://docs.oracle.com/javase/6/docs/technotes/guides/security/crypto/CryptoSpec.html#PBEEx">Java
   * Cryptography Architecture (JCA) Reference Guide</a> Also wipes raw data.
   */
  ...
  public boolean equals(final Object obj) {
    ...
    if (!Objects.equals(accountNumberInfo, other.accountNumberInfo)) {
      return false;
    }
    if (!Objects.equals(issuerIdentificationNumber, other.issuerIdentificationNumber)) {
      return false;
    }
    if (!Objects.equals(lastFourDigits, other.lastFourDigits)) {
      return false;
    }
    return true;
  }
  ...
  public int hashCode() {
    ...
    result = prime * result + (accountNumberInfo == null ? 0 : accountNumberInfo.hashCode());
    result =
        prime * result
            + (issuerIdentificationNumber == null ? 0 : issuerIdentificationNumber.hashCode());
  ...
  public String toString() {
    if (hasAccountNumber()) {
      ...
      return String.format("%s-%s", accountNumberInfo.getCardBrand(), getLastFourDigits());
    } else {
